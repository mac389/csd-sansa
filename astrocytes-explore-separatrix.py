...
def minimal_example():
    ...
    # Neuron model
    equations = Equations('''
        dV/dt = (-(V-Vr)+x+g_gap*(u-N*V)+g_astro*(u_astro-N*v))*(1./taum)              : volt
        dx/dt = (-x+y)*(1./taupsp)                               : volt
        dy/dt = -y*(1./taupsp)+25.27*mV/ms+(39.24*mV/ms**0.5)*xi : volt
        du/dt = (N*Vr-u)/tau : volt # input from other neurons
        du_astro/dt = (N*Vr-u_astro)/(3*tau) : volt # input from other astrocytes
        ''')
...
#                  threshold=Vt,reset=Vr,refractory=1*ms)
    ...
    run(100 * ms)
    # Plot result
    raster_plot(color='k',showgrouplines=True, *monitors)
    show()
...
class DefaultNetwork(Network):
    ...
    def plot(self):
        ...
        raster_plot(color='k', markersize=3,newfigure=False,
                   showgrouplines=True, spacebetweengroups=0.2, grouplinecol=(0.2,0.2,0.2),
                   *self.mon)
        artist.adjust_spines(ax)
        ax.set_ylabel(r'\textbf{\textsc{Neurons}}')
        ax.set_xlabel(r'\textbf{\textsc{Time}}')
        ax.set_ylim(ymin=0,ymax=default_params.num_layers)
        plt.tight_layout()
...
with open('record-astrocytes','a') as f:
    ...
    try:
        ...
        '''
        savefig('astrocyte-separatrix-%d.png'%(frac*10),dpi=300)
